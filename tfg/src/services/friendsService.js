...
export function searchUsers(query) {
  return supabase
    .from('profiles')
    .select('id, display_name, username, avatar_url, completed_count, current_streak')
...
export async function fetchFriendUpcomingEvents(friendUserId) {
  const { data: profile } = await supabase
    .from('profiles')
    .select('show_events_to_friends')
    .eq('id', friendUserId)
    .single()

  if (!profile?.show_events_to_friends) return { data: [], error: null }

  const today = new Date().toISOString().split('T')[0]
  const { data, error } = await supabase
    .from('event_participants')
    .select('events(id, title, event_date, event_time, current_participants, total_places, status, city, address, lat, lng, level, sports(name))')
