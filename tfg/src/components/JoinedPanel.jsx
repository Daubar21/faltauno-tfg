...
import { FiAlertTriangle, FiCalendar, FiClock, FiMapPin, FiStar, FiX } from 'react-icons/fi'
...
export function JoinedPanel({ joinedEvents, waitlistEvents = [], onCancel, onLeaveWaitlist, onRate, onClose }) {
  ...
  async function handleLeaveWaitlist(eventId) {
    setLeavingWaitlist(eventId)
    try {
      await onLeaveWaitlist?.(eventId)
    } finally {
                ...
                return (
                  <li key={event.id} className="waitlist-item">
                    <div className="waitlist-item-info">
                      <span className="waitlist-sport">
                        <SportIcon aria-hidden="true" /> {event.sport}
                      </span>
                      <p className="waitlist-title">{event.title}</p>
                      <p className="waitlist-meta">
                        <FiCalendar aria-hidden="true" /> {event.date}
                      </p>
                      <p className="waitlist-meta">
                        <FiMapPin aria-hidden="true" /> {event.city}
                      </p>
                      <span className="waitlist-badge">🔴 Completo · {event.currentParticipants}/{event.totalPlaces}</span>
                    </div>
                    <button
                      type="button"
                      className="waitlist-leave-btn"
                      onClick={() => handleLeaveWaitlist(event.id)}
                      disabled={leavingWaitlist === event.id}
                    >
                      <FiX aria-hidden="true" />
                      {leavingWaitlist === event.id ? 'Saliendo…' : 'Salir de la espera'}
                    </button>
                  </li>
                )
              })}
